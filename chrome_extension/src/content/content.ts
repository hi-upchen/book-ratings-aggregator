...
import { cleanBookTitle,
	...
	extractPriceAndCurrency } from './kobo.ts';
	...
	const handlePchomeBookDetail = () => {
		...
		if (bookInfoEl) {
			Array.from(bookInfoEl.children).forEach((child) => {
				author = pchome.extractAuthorFromBookInfo(child.textContent.trim()) ?? author
				isbn = pchome.extractISBNFromBookInfo(child.textContent.trim()) ?? isbn
			});
		}
		...
		chrome.runtime.sendMessage({
			...
			data: { 
				...
				book: {
					...
					price,
					currency,
					format: originalBookTitle.indexOf('電子書')>=0 ? 'ebook' : 'physical',
					isbn
				}
			}
		}, {}, (response) => {
